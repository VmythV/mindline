...
| 协同     | POST   | `/maps/:mapId/commands`（服务端写通道）     | Editor+                 | 路线2 | 完整   |
...
{
  "items": [
    { "id": "p_1", "name": "新产品立项", "parentId": null, "mapId": "m_1", "archived": false },
  ],
  "nextCursor": null,
}
```
...
{
  "migrationId": "mig_x",
  "status": "running|done|failed|rolledback",
  "processed": 120,
  "total": 126,
...
### POST /maps/:mapId/commands （服务端写通道）【完整】 · 路线2

非浏览器客户端（CLI / AI）执行命令层命令的入口。api 收到后作为 Hocuspocus 客户端连入 collab，
在协同文档（Y.Doc）上执行命令层（`MapRepository`），变更经 provider 广播给所有在线客户端，
语义事件复用 `POST /maps/:mapId/changes` 的落库逻辑（`ChangesService.append`）。命令层仍是 Y.Doc 唯一写入口（Yjs §11）。Editor+。

```jsonc
// 请求：commands 为命令层命令数组（可辨识联合，见 @mindline/shared Command）
{
  "commands": [
    { "kind": "createChild", "parentId": "n_root", "title": "新子节点", "type": "task" },
    { "kind": "rename", "nodeId": "n_42", "title": "改名" },
    { "kind": "setField", "nodeId": "n_42", "field": "priority", "value": "高" },
    { "kind": "move", "nodeId": "n_42", "newParentId": "n_9" },
    { "kind": "delete", "nodeId": "n_7" },
    {
      "kind": "applyProposal",
      "proposal": {
        /* SSE 重建的 Proposal */
...
// 响应 200 { "created":["n_01HX..."], "eventCount":3 }
```

错误：`403`（非 Editor+）、`404`（map 不存在/跨租户/非成员）、`503`（collab 不可达）、`400`（非法命令 kind）。
...
{
  "channelId": "ch_1",
  "payloadType": "node|milestone|summary",
  "refId": "n_42",
  "note": "请相关同学关注",
} // refId: node→nodeId, milestone→ms_id, summary→mapId
...
```jsonc
{
  ...
  "tags": ["支付"],
  "collaborators": ["u_2"],
  "data": { "priority": "高", "due": 1730600000000 },
  "links": [{ "kind": "reference|subproject", "targetId": "n_99|p_2" }],
  "private": false,
}
```
...
_（契约总览结束。任何端点变更请同步更新本文件与对应详设，保持单一事实来源。）_
