...
function NodeCardImpl({ id, data, selected }: NodeProps) {
  ...
  if (shadow) {
    ...
    return (
      <div
        className={`relative px-3 py-2 rounded-lg border-2 border-dashed text-sm w-[180px] min-h-10 ${
          accepted
  ...
  if (filterStatus === 'path') {
    return (
      <div className="relative px-3 py-2 rounded-lg border border-dashed border-slate-300 bg-slate-50/60 opacity-50 text-sm w-[180px] min-h-10">
        <Handle type="target" position={Position.Left} className="!bg-slate-300" />
  ...
  if (isPrivate) {
    return (
      <div
        className={`relative px-3 py-2 rounded-lg border bg-slate-100 shadow-sm text-sm w-[180px] min-h-10 ${
  ...
  return (
    <div
      className={`relative px-3 py-2 rounded-lg border bg-white shadow-sm text-sm w-[180px] min-h-10 ${
        selected ? 'border-blue-500 ring-2 ring-blue-200' : 'border-slate-200'
      } ${cardData.recent ? 'mindline-node-enter' : ''}`}
    ...
    >
      ...
      {node.private && (
        <span
          className="absolute top-0.5 left-0.5 text-[9px] text-slate-400"
          title="此节点已标记为私有"
        >
          🔒
        </span>
